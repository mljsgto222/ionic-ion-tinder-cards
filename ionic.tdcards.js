(function (ionic) {
  ...
  var getMatrix = function(scaleX, scaleY, translateX, translateY){
  ...
  angular.module('ionic.contrib.ui.tinderCards', ['ionic'])
      ...
      return {
        ...
        compile: function (element, attr) {
          return function ($scope, $element, $attr, swipeCards) {
            ...
            el.style.transform = el.style.webkitTransform = getMatrix(1, 1, 0, 0);
            el.style.opacity = 1;
    ...
    .directive('tdCards', ['$rootScope', '$timeout', function ($rootScope, $timeout) {
      var CARD_DURATION = 28;
      var OPACITY_DURATION = 0.2;
      var SCALE_DURATION = 0.08;
      var getOpacity = function(index){
        return -(OPACITY_DURATION * Math.max(index, 0)) + 1;
      };
      var getScale = function(index){
        return -(SCALE_DURATION * Math.max(index, 0)) + 1
      }
      var _scale;
      var setCardStyle = function(card, index){
        _scale = getScale(index);
        card.style.transform = card.style.webkitTransform = getMatrix(_scale, _scale, 0, -(index * CARD_DURATION));
        card.style.opacity = getOpacity(index);
      };
      return {
        restrict: 'E',
        ...
        controller: ['$scope', '$element', function ($scope, $element) {
          ...
          this.pushCard = function (card) {
            setCardStyle(card, size);
            card.style.zIndex = - size;
            size += 1;
          };
          ...
          var opacity;
          var scale;
          var bringCardUp = function (card, amt, index) {
            var newOpacity, newTop, newScale;
            max = index * CARD_DURATION;
            opacity = getOpacity(index);
            scale = getScale(index);
            newOpacity = Math.min(getOpacity(index - 1), Math.max(opacity, opacity + (OPACITY_DURATION * Math.abs(amt))));
            newScale = Math.min(getScale(index - 1), Math.max(scale, scale + (SCALE_DURATION * Math.abs(amt))));
            newTop = Math.max((index - 1) * CARD_DURATION, Math.min(max, max - (CARD_DURATION * Math.abs(amt))));
            card.style.transform = card.style.webkitTransform = getMatrix(newScale, newScale, 0, -newTop);
          ...
          this.partial = function (amt) {
            ...
            secondCard && bringCardUp(secondCard, amt, 1);
            thirdCard && bringCardUp(thirdCard, amt, 2);
          };
        }]
      }
    }])
